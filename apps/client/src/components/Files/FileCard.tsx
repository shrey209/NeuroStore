...
const FileCard: React.FC<FileCardProps> = ({
  ...
  const [newAccessType, setNewAccessType] = useState<"gmail" | "github_id">(
    "gmail"
  );
  const [newAccessLevel, setNewAccessLevel] = useState<"read" | "write">(
    "read"
  );
...
const handleRenameSave = async () => {
  ...
  const handleDelete = async () => {
    try {
      console.log("🗑️ Deleting file:", file.file_id);

      const res = await axios.delete(
        `${BASE_URL}/api/file/delete/${file.file_id}`,
        {
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          {/* Access Level Badge */}
          <div
            className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium border ${getAccessLevelColor()}`}
          >
            {getAccessLevelIcon()}
        ...
        >
          ...
          >
              ...
              {localIsPublic ? (
                <div className="space-y-3">
                  <p className="text-sm text-gray-600">
                    This file will be public. Anyone with the link can view it
                    (read-only access).
                  </p>
                  {publicLink && (
                ...
                <p className="text-sm text-gray-600">
                  Toggle to make this file public and generate a shareable link
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
                    {localSharedWith.length} user
                    {localSharedWith.length !== 1 ? "s" : ""}
                  </span>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-3">
                  <select
                    value={newAccessType}
                    onChange={(e) =>
                      setNewAccessType(e.target.value as "gmail" | "github_id")
                    }
                ...
                <div className="flex items-center space-x-3">
                  <select
                    ...
                    onChange={(e) =>
                      setNewAccessLevel(e.target.value as "read" | "write")
                    }
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                  >
            ...
            <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-lg mt-4">
              ...
              <p className="mb-1">
                <strong>Shared:</strong> Specific users with defined permissions
              </p>
              <p>
                <strong>Private:</strong> Only you can access this file
              </p>
            </div>
          </div>
        </div>

...
import { FileDataDTO, ChunkData } from "@neurostore/shared/types";
      ...
      try {
        ...
        socket.onmessage = (event) => {
          try {
            ...
            if (receivedData.err) {
              console.error(
                `❌ Error in chunk ${receivedData.chunk_no}:`,
                receivedData.err
              );
            } else {
              console.log(
                `✅ Chunk ${receivedData.chunk_no} received successfully.`
              );
            }
          } catch (e) {
        ...
        socket.onclose = () => {
          ...
          const combined = new Blob(byteArrays, {
            type: getIframeMimeType(data.file_extension),
          });
